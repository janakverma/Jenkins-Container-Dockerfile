...
## Specify jenkins user and group as Argument with uid, gid and http port to run jenkins.
ARG user=jenkins
ARG group=jenkins
ARG uid=1000
ARG gid=1000
...
RUN service jenkins start && sleep 30 && sed -i s@jenkins_password@"$(cat /var/lib/jenkins/secrets/initialAdminPassword)"@g /etc/jenkins_jobs/jenkins_jobs.ini
...
RUN cd ~/ && service jenkins start && sleep 30 && jenkins-jobs update jobs
